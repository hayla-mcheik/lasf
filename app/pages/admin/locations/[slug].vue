      ...
      <div class="site-branding mb-4 d-flex justify-content-between align-items-end">
        <div>
          ...
          <p class="text-muted small mb-0">
            ...
            {{ location.type || 'Unknown Type' }} | إحداثيات: {{ location.latitude }}, {{ location.longitude }}
          </p>
        </div>
          ...
          <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div class="d-flex align-items-center">
              ...
              <div class="metric-group text-center">
                <h2 class="h1 fw-black mb-0">{{ Math.round(weather?.main?.temp || 0) }}°</h2>
...
></iframe>
          ...
          <div class="map-controls p-3 position-absolute bottom-0 start-0 w-100 d-flex justify-content-between align-items-end">
            <div class="glass-pill px-3 py-2 fw-bold small">
              <i class="bi bi-compass me-2"></i>{{ windDirection }} ({{ weather?.wind?.deg ?? 'N/A' }}°)
            </div>
        ...
        <div class="col-lg-4">
          <div class="card border-0 shadow-sm rounded-4 p-4 h-100 bg-dark text-white">
            ...
            <div class="d-flex justify-content-between py-2 border-bottom border-secondary">
              ...
              <span class="fw-bold">{{ weather?.main?.pressure ?? 'N/A' }} hPa</span>
            </div>
            <div class="d-flex justify-content-between py-2">
              <span class="small opacity-75">VFR Visibility</span>
              <span class="fw-bold">{{ weather?.visibility
...
const {
  ...
  async () => {
    try {
      return await $fetch(
        `${config.public.apiBase}/admin/flying-locations/${route.params.slug}`,
        {
          headers: {
...
const location = computed(() => locationResponse.value?.data || null)
...
|--------------------------------------------------------------------------
*/
const windKt = computed(() => {
  return Math.round(
    (weather.value?.wind?.speed || 0) * 1.94384
  )
})

const windGustKt = computed(() => {
  const gust = weather.value?.wind?.gust

  if (gust) {
    return Math.round(gust * 1.94384)
  }

  return Math.round(windKt.value * 1.3)
})

const isSafeToFly = computed(() => {
  return (
    windKt.value < 15 &&
    windGustKt.value < 22
  )
  ...
  meta: [
    {
      ...
      content: location.value
        ? `Live aviation weather and wind forecast for ${location.value.name}`
        : 'Live aviation weather information'
    }
  ]
}))
</script>
