...
const savePilot = async () => {
  try {
    ...
    // Laravel Method Spoofing for File Uploads in PUT
    if (isEdit) {
      formData.append('_method', 'PUT')
    }

    const response = await $fetch(url, {
      method: 'POST', // Always POST when using FormData with files
      headers: {
        'Authorization': `Bearer ${authStore.token}`,
        'Accept': 'application/json'
      },
    ...
    alert(isEdit ? 'Pilot updated successfully!' : 'Pilot added successfully!')
    closeModal()
    await fetchPilots()
    ...
    if (err.data && err.data.errors) {
      ...
      formErrors.value = [err.message || 'An error occurred']
    }
  } finally {
    saving.value = false
  }
